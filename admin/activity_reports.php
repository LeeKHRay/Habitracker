...
function printTableRows($reports, $resolved) {
    ...
    while ($report = $reports->fetch_assoc()) {
        $deleted = !isset($report['activity_name']);
        ...
        echo '<td class="align-middle">'.$report['report_activity_name'].'</td>';
        ...
        if ($resolved) {
            ...
            if (!$deleted && !$report['dismissed']) {
                echo '<td class="align-middle"><button class="deleteActivity btn btn-link" data-id="'.$report['activity_id'].'" data-username="'.$report['owner'].'">Delete</button></td>';
...
$sql = "SELECT r.report_id, r.report_time, r.reporter, r.owner, r.activity_id, 
            r.activity_name AS report_activity_name, a.activity_name, r.reason, r.dismissed
        FROM report r 
        LEFT JOIN activity a 
        ON r.activity_id = a.activity_id 
        WHERE r.resolved = FALSE AND r.report_type = 'activity' ORDER BY r.report_time";
...
$sql = "SELECT r.report_id, r.report_time, r.reporter, r.owner, r.activity_id, 
        ...
        LEFT JOIN activity a 
        ON r.activity_id = a.activity_id 
        WHERE r.resolved = TRUE AND r.report_type = 'activity' ORDER BY r.report_time";
